...
export function storyToHtml(input: StoryHtmlInput): string {
  ...
  if (pictures && pictures.length > 0) {
    const figures = pictures
      .map((p, i) => {
        const caption = p.caption?.trim() || `Picture ${i + 1}`;
        return `<figure><figcaption>${escapeHtml(caption)}</figcaption><img src="${escapeHtml(p.dataUrl)}" alt="${escapeHtml(caption)}"></figure>`;
      })
      .join('');
    sections.push(`<section><h2>Pictures</h2><div class="ui-grid">${figures}</div></section>`);
  }
...
<nav class="ark-tabs">
  ...
  <label for="t-mockup">Interactive GUI<span class="ark-tab-badge">✷</span></label>
</nav>
