...
Production runs as a **single Azure App Service** (Linux, Node 20) where one
Node process serves both `/api/*` and the React SPA. See
`docs/deployment.md` for the one-time `az` CLI provisioning runbook.

**CI/CD:** every push to `main` triggers `.github/workflows/deploy.yml`, which
builds the frontend, copies `src/frontend/dist/` into `src/backend/public/`,
installs backend production deps, zips the backend folder, and zip-deploys
via `azure/webapps-deploy@v3` using the `AZURE_WEBAPP_PUBLISH_PROFILE` secret.

**Secrets:** `ANTHROPIC_API_KEY`, `AZURE_DEVOPS_ORG`, `AZURE_DEVOPS_PROJECT`,
`AZURE_TENANT_ID`, `DATA_DIR=/home/data` all live in App Service Application
Settings — never in the repo, never in the frontend bundle. To rotate a key:
update the App Setting and `az webapp restart`; no redeploy needed.

**Persistence:** drafts persist across redeploys because `DATA_DIR=/home/data`
points outside `wwwroot` (the latter is wiped on each deploy). The
`draftStore.ts` falls back to `<backend>/data` in local dev.

**Built artifact:** `src/backend/public/` is generated by the deploy workflow
(or by running `npm run build` in `src/frontend` + copying `dist/`). It is
