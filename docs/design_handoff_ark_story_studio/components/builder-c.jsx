// Story builder — variant C: Storyboard / canvas approach
// Visual index cards on a board representing each story component; drag-arrange & AI fills gaps.

function StoryBuilderC({ onSubmit, onBack }) {
  ...
  return (
    <div className="ark-root" style={{ width: '100%', height: '100%', background: ARK_TOKENS.bg, display: 'flex', flexDirection: 'column' }}>
      <TopBar
        breadcrumbs={['Support Platform', 'Billing', 'Story canvas']}
        rightActions={
      ...
      <div style={{ padding: '16px 24px', borderBottom: `1px solid ${ARK_TOKENS.border}`, background: ARK_TOKENS.surface, display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: ARK_TOKENS.inkMuted, marginTop: 2, display: 'flex', gap: 10 }}>
            <span>7 cards</span>
            <span>·</span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 6, height: 6, background: ARK_TOKENS.success, borderRadius: 3 }} /> Complete story</span>
            <span>·</span>
      ...
      {/* Canvas */}
      <div className="ark-scroll" style={{ flex: 1, overflow: 'auto', position: 'relative', background: `
        radial-gradient(circle, rgba(0,0,0,0.08) 1px, transparent 1px)
      `, backgroundSize: '20px 20px', padding: 40 }}>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 28, minWidth: 980, maxWidth: 1400, margin: '0 auto' }}>
          {/* Row 1: narrative */}
          <IdxCard data={cards.persona} />
          <IdxCard data={cards.problem} />
          <IdxCard data={cards.want} />
          <IdxCard data={cards.outcome} />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 28, minWidth: 720, maxWidth: 1100, margin: '0 auto' }}>
          <IdxCard data={cards.ac1} />
          <IdxCard data={cards.ac2} />
          <IdxCard data={cards.ac3} />
        </div>
...
function IdxCard({ data }) {
  const I = { user: Ico.user, warn: Ico.warn, bolt: Ico.bolt, heart: Ico.heart, check: Ico.check }[data.icon];
  return (
    <div style={{
    ...
    onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = ARK_TOKENS.shadow2; }}
    onMouseLeave={(e) => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = ARK_TOKENS.shadow1; }}
    >
      ...
      <div style={{ display: 'flex', gap: 4, marginTop: 4, borderTop: `1px solid ${ARK_TOKENS.border}`, paddingTop: 8 }}>
        <button style={{ border: 'none', background: 'transparent', color: ARK_TOKENS.inkSubtle, cursor: 'pointer', padding: 4, borderRadius: 3, fontSize: 11, display: 'flex', alignItems: 'center', gap: 3 }}>
          <Ico.edit size={11} /> Edit
        </button>
        <button style={{ border: 'none', background: 'transparent', color: ARK_TOKENS.ai, cursor: 'pointer', padding: 4, borderRadius: 3, fontSize: 11, display: 'flex', alignItems: 'center', gap: 3 }}>
...
function LegendTag({ color, label }) {
  return (
...
Object.assign(window, { StoryBuilderC, IdxCard });
