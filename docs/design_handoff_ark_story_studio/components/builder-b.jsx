// Story builder — variant B: AI Coach conversation
// Chat on the left, story materializes on the right as AI extracts structured fields.

function StoryBuilderB({ onSubmit, onBack }) {
  const [messages, setMessages] = React.useState([
    ...
    { role: 'ai', text: "Perfect — that's everything I need. I've drafted a story on the right. Take a look and tell me what to change.", showPreview: true },
  ]);
  ...
  const send = () => {
    ...
    setTimeout(() => {
      setTyping(false);
      setMessages(m => [...m, { role: 'ai', text: "Got it — I've updated the story. Anything else to tighten up, or should we push to Azure?", showPreview: true }]);
    }, 1400);
  };
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
          ...
          <div style={{ padding: '16px 28px 20px', borderTop: `1px solid ${ARK_TOKENS.border}`, background: ARK_TOKENS.surface }}>
            <div style={{ maxWidth: 600, margin: '0 auto' }}>
              <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
                {['Add an edge case', 'Make it smaller', 'Suggest acceptance criteria', 'Check INVEST'].map(s => (
                  <button key={s} onClick={() => setInput(s)} style={{
                    border: `1px solid ${ARK_TOKENS.border}`, background: ARK_TOKENS.surface,
                    padding: '5px 12px', borderRadius: 14, fontSize: 12, color: ARK_TOKENS.inkMuted,
              ...
              <div style={{
                ...
                <textarea
                  ...
                  rows={2}
                  style={{ flex: 1, border: 'none', background: 'transparent', resize: 'none', outline: 'none', fontFamily: 'inherit', fontSize: 14, lineHeight: 1.5 }}
                />
        ...
        <div style={{ flex: '0 0 460px', background: ARK_TOKENS.surface, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${ARK_TOKENS.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
            <AzureMark size={16} />
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 600 }}>Story being built</div>
              <div style={{ fontSize: 11, color: ARK_TOKENS.inkSubtle, display: 'flex', alignItems: 'center', gap: 4 }}>
                <span style={{ width: 6, height: 6, background: ARK_TOKENS.success, borderRadius: 3 }} /> Auto-saving to draft
              </div>
            </div>
            <Btn size="sm" variant="ghost" icon={<Ico.edit size={12} />}>Edit manually</Btn>
          </div>
          <div className="ark-scroll" style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
            <WorkItemPreview {...storyData} />
          </div>
        </div>
      </div>
    </div>
  ...
  if (msg.role === 'ai') {
    return (
      <div style={{ display: 'flex', gap: 12, animation: 'ark-fadein 0.3s' }}>
        <div style={{ width: 32, height: 32, borderRadius: 16, background: `linear-gradient(135deg, ${ARK_TOKENS.ai} 0%, #b57edc 100%)`, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <Ico.sparkle size={14} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11, color: ARK_TOKENS.inkSubtle, fontWeight: 600, marginBottom: 4 }}>ARK COACH</div>
          {msg.typing ? (
            ...
            <div style={{ fontSize: 14, lineHeight: 1.55, color: ARK_TOKENS.ink, whiteSpace: 'pre-wrap' }}
  ...
  return (
    <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', animation: 'ark-fadein 0.3s' }}>
      <div style={{ maxWidth: '85%' }}>
        <div style={{ fontSize: 11, color: ARK_TOKENS.inkSubtle, fontWeight: 600, marginBottom: 4, textAlign: 'right' }}>YOU</div>
        <div style={{ background: ARK_TOKENS.azure, color: '#fff', padding: '10px 14px', borderRadius: 12, borderBottomRightRadius: 4, fontSize: 14, lineHeight: 1.5 }}>
          {msg.text}
        </div>
      </div>
...
Object.assign(window, { StoryBuilderB, ChatMsg });
